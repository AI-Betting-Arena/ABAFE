...
export default async function EventsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string }> }) {
  ...
  // Filter buttons / weekly navigation
  const weekOptions = [
    ...
    <main className="max-w-5xl mx-auto px-4 py-8">
      ...
      <header className="mb-10">
        ...
        <div className="flex items-center justify-center gap-2 mb-12">
          <Link href={{ pathname: "/events", query: { ...params, status: 'all' } }} scroll={false} className={`px-6 py-2.5 rounded-lg bg-cyan-500/20 border border-cyan-500/50 text-cyan-400 font-medium${status === 'all' ? '' : ' hover:border-cyan-500/70 hover:text-cyan-300 transition-all'}`}>All</Link>
          <Link href={{ pathname: "/events", query: { ...params, status: 'open' } }} scroll={false} className={`px-6 py-2.5 rounded-lg bg-slate-800/30 border border-slate-700 text-slate-400 hover:border-slate-600 hover:text-slate-300 transition-all${status === 'open' ? ' border-cyan-500/50 text-cyan-400 bg-cyan-500/20 font-medium' : ''}`}>Open</Link>
          <Link href={{ pathname: "/events", query: { ...params, status: 'live' } }} scroll={false} className={`px-6 py-2.5 rounded-lg bg-slate-800/30 border border-slate-700 text-slate-400 hover:border-slate-600 hover:text-slate-300 transition-all${status === 'live' ? ' border-cyan-500/50 text-cyan-400 bg-cyan-500/20 font-medium' : ''}`}>Live</Link>
          <Link href={{ pathname: "/events", query: { ...params, status: 'finished' } }} scroll={false} className={`px-6 py-2.5 rounded-lg bg-slate-800/30 border border-slate-700 text-slate-400 hover:border-slate-600 hover:text-slate-300 transition-all${status === 'finished' ? ' border-cyan-500/50 text-cyan-400 bg-cyan-500/20 font-medium' : ''}`}>Finished</Link>
        </div>
      </header>
      {/* League sections (show all without filter) */}
